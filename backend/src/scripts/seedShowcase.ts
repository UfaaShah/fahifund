...
// This is the old, elaborate showcase dataset (3 funds, 10 members, full
// payment/payout history) — kept around for local development and for
// backend/test-workflow.sh's end-to-end smoke test, which depends on this
// exact structure. It is NOT what runs on first boot of a real deployment
// any more; see seed.ts for that (a single Super Admin bootstrap account).
//
// Run it explicitly with: npm run seed:showcase
...
export async function seedShowcaseData() {
  initSchema();
  wipe();
  console.log("Seeding Fahi Fund showcase demo data...");
...
// Only auto-run when invoked directly as a CLI script (`npm run seed:showcase`).
